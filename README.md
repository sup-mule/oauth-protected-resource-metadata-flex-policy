...
|----------|------|----------|-------------|
| `resourceUrl` | string (URI) | Yes | The protected resource's resource identifier |
| `authorizationServers` | array of strings | No | OAuth authorization server issuer identifiers |
| `scopesSupported` | array of strings | No | Scope values used in authorization requests |

## Example Response

When the policy is applied and a client requests `GET /.well-known/oauth-protected-resource`:
...
1. **Setup** (requires Anypoint authentication): `make setup`
2. **Build**: `make build`
3. **Run locally**: `make run` (requires Flex Gateway registration in Local Mode)
4. **Test**: `curl http://localhost:8081/.well-known/oauth-protected-resource`
